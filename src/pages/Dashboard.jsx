...
import { mockComplaintApi, mockChatbotApi, mockSmsApi } from '../lib/mockApi';
import {
  UserIcon,
  EnvelopeIcon,
  AcademicCapIcon,
...
function NewComplaintForm({ user }) {
  ...
  const classifyComplaint = async () => {
    ...
    try {
      const data = await mockComplaintApi.classifyComplaint(`${formData.title} ${formData.description}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await mockComplaintApi.submitComplaint({
  ...
  const handleVoiceComplaintSubmit = async (voiceData) => {
    ...
    try {
      const data = await mockComplaintApi.submitComplaint({
        title: voiceData.description.substring(0, 100).trim() || 'Voice Complaint',
        description: voiceData.description.trim(),
        userId: user?.id
      });
...
function AutoRaisePopup({ isOpen, onClose, user }) {
  ...
  const generateComplaintDetails = async (userInput) => {
    ...
    try {
      const aiData = await mockChatbotApi.generateComplaint(userInput, user);

      if (aiData.success) {
        // Extract title and description from AI response
        const aiResponse = aiData.bot_response;
  ...
  const classifyComplaint = async (title, description) => {
    ...
    try {
      const data = await mockComplaintApi.classifyComplaint(`${title} ${description}`);
  ...
  const submitComplaint = async () => {
    ...
    try {
      const data = await mockComplaintApi.submitComplaint({
        title: complaintDetails.title.trim(),
        description: complaintDetails.description.trim(),
        userId: user?.id
      });
...
function AIAssistantPopup({ isOpen, onClose, user }) {
  ...
  const sendMessage = async (message) => {
    ...
    try {
      const data = await mockChatbotApi.chat(message, user);
...
function TrackProgressSection({ user }) {
  ...
  const searchComplaint = async () => {
    ...
    try {
      const data = await mockComplaintApi.checkStatus(ticketId.trim());
  ...
  const submitFeedback = async (complaintId) => {
    ...
    try {
      const data = await mockComplaintApi.submitFeedback(complaintId, feedbackForm);
...
function ReportsSection({ user }) {
  ...
  const fetchComplaints = async (page = 1) => {
    ...
    try {
      const data = await mockComplaintApi.listComplaints({
  ...
  const fetchStats = async () => {
    try {
      const data = await mockComplaintApi.getStats();
...
function FeedbackSection({ user }) {
  ...
  const fetchResolvedComplaints = async () => {
    try {
      const data = await mockComplaintApi.listComplaints({ userId: user?.id, perPage: 100 });
      
      if (data.success) {
        const resolved = data.data.complaints.filter(complaint =>
          complaint.status === 'resolved' || complaint.status === 'closed'
        );
...
function CommunitySection({ user }) {
  ...
  const fetchCommunityComplaints = async () => {
    try {
      const data = await mockComplaintApi.listCommunityComplaints({ page: currentPage, perPage: 5 });
...
function SettingsSection({ user }) {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const data = await mockSmsApi.savePreferences(user?.id, {
  ...
  const handleSMSRegistration = async () => {
    ...
    try {
      const data = await mockSmsApi.register(user?.id, phoneNumber);
...
function Dashboard() {
  ...
  console.log('📊 Dashboard component loaded, user:', user, 'loading:', loading);
